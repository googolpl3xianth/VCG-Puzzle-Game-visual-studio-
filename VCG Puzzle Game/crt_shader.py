...
class Graphic_engine:
    def __init__(self, screen, style = 1, cpu_only=False, fullscreen=False):
        pygame.init()
        self.VIRTUAL_RES = screen.get_size()
        ...
        if not(self.cpu_only):
            ...
            self.prog = self.ctx.program(
                vertex_shader=open(resource_path(r'VCG Puzzle Game\shaders\VERTEX_SHADER.glsl')).read(),
                fragment_shader=open(resource_path(r'VCG Puzzle Game\shaders\FRAGMENT_SHADER.glsl')).read(),
            )
